...
def _required(state, pid, target, dist: int) -> int:
    ...
    A rival bloc landing on the *same* turn as us is still folded into the
    garrison here, though the engine has it fight us first. That is stale and
    known: correcting it changes about one price in a thousand and measures
    null. See "The attack side of the pile-up" in `docs/bot-design.md`.
    """
    ships = target.ships
